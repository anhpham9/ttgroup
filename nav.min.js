! function (s) {
    ...
    s.fn.vegasMenu = function (a) {
        ...
        return n.addClass(d),
            function () {
                // Thêm phần kiểm tra có chứa thẻ <i> không
                r.find(".dropdown-mega > a, .dropdown > a").each(function () {
                    var a = s(this).text();
                    // Kiểm tra nếu có <i> trong thẻ <a> thì thêm class cho thẻ đó
                    if (s(this).find('i').length > 0) {
                        var icon = s(this).find('i').prop('outerHTML'); // Lấy thẻ <i> bên trong
                        s(this).html('<span class="fa-wrapper">' + icon + ' <span class="fa-text">' + a + '</span></span>'); // Thêm <i> và văn bản vào
                    } else {
                        s(this).html(a + '<span class="toggle"></span>'); // Không có <i>, thêm <span class="toggle">
                    }
                }), e.prepend('<a href="#openNav" class="' + l + '"><span></span></a>'), r.append('<div class="' + o + '"><div class="' + o + '__close">&times;</div><div class="' + o + '__content"></div></div>'), r.append('<div class="' + t + '"></div>');
